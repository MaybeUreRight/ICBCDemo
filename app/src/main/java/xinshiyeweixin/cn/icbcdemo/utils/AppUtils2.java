...
public final class AppUtils2 {
    ...
    public static boolean isAppRoot() {
        ...
        if (result.errorMsg != null) {
            Log.d("AppUtils2", "isAppRoot() called" + result.errorMsg);
        }
        return false;
    }
    ...
    public static void getAppDetailsSettings(final String packageName) {
        if (isSpace(packageName)) return;
        Utils.getApp().startActivity(IntentUtils.getLaunchAppDetailsSettingsIntent(packageName, true));
    }
    ...
    public static boolean isAppForeground() {
        ActivityManager manager =
                (ActivityManager) Utils.getApp().getSystemService(android.content.Context.ACTIVITY_SERVICE);
